...
export default function UsersPage() {
  ...
  const handleEditUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.message || "Greška pri izmeni korisnika");
      }
    } catch {
  ...
  const handleOpenEditModal = (userId: string) => {
    ...
    setEditFormData({
      charismaId: userToEdit.charismaId || "",
      username: userToEdit.username,
      name: userToEdit.name,
      role: userToEdit.role,
      depot: userToEdit.depot || "",
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => router.push(`/dashboard/2fa-setup?userId=${u.id}`)}
                            disabled={actionLoading}
                            className="px-3 py-1.5 bg-indigo-100 text-indigo-700 hover:bg-indigo-200 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Upravljaj 2FA"
                          >
                            2FA
                          </button>
                          <button
                            ...
                            disabled={actionLoading}
                            className="px-3 py-1.5 bg-blue-100 text-blue-700 hover:bg-blue-200 rounded-lg text-xs font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Izmeni
                          </button>
                          <button
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-gray-900">Izmeni korisnika</h3>
              <button
                onClick={() => {
                  setShowEditModal(false);
                  setSelectedUserId(null);
            ...
            <form onSubmit={handleEditUser} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  value={editFormData.charismaId}
                  onChange={(e) => setEditFormData({ ...editFormData, charismaId: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="npr. EMP001"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, depot: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="npr. Central"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditFormData({ ...editFormData, role: e.target.value as "gospodar" | "super_user" | "technician" })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  <option value="technician">Serviser</option>
                  <option value="super_user">Administrator</option>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Lozinka se menja samo ako popunite ovo polje
                </p>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {actionLoading ? "Čuvanje..." : "Sačuvaj izmene"}
                </button>
              </div>
            </form>
          </div>
